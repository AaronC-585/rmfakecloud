...
export function applyColorOverrides(themeDoc, overrides) {
...
export function transformWithXSLT(xmlDoc, xslDoc) {
...
export function transformToText(xmlDoc, xslDoc) {
  if (typeof XSLTProcessor === "undefined") {
    throw new Error("XSLTProcessor not supported");
  }
  const proc = new XSLTProcessor();
...
export function defaultChromePrefs() {
  return {
    style: "flat",
    folderColor: "#d4b483",
    tabColor: "#c4a574",
...
function chromeFromElement(el) {
  ...
  if (!el) return d;
  return {
    style: el.getAttribute("style") || d.style,
...
export function defaultConnectPrefs() {
  return {
    gaugeType: "circle",
    trackColor: "#3a3f44",
    fillColor: "#EE7B30",
...
export function parseLayoutDocument(layoutDoc) {
  ...
  return {
    ...
    nav: {
      ...
      items: [
        ...
        { id: "screenshare", visible: false, adminOnly: false },
...
export function applyFormFactor(themeDoc, formFactor) {
...
export function themeXMLFromEditor(state) {
...
export function editorStateFromXML(xmlString) {
