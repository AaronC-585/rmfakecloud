...
// Builtin is a built-in template preview.
type Builtin struct {
	ID   string
	Name string
}
...
var builtins = []struct {
	...
	{id: "blank", name: "Blank", svg: blankSVG()},
	{id: "lined", name: "Lined", svg: linedSVG()},
	{id: "grid", name: "Grid", svg: gridSVG()},
	{id: "dotted", name: "Dotted", svg: dottedSVG()},
}
...
// ListBuiltins returns built-in template id/name pairs.
func ListBuiltins() []Builtin {
	out := make([]Builtin, 0, len(builtins))
	for _, b := range builtins {
		out = append(out, Builtin{ID: b.id, Name: b.name})
	}
	return out
}
