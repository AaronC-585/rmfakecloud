...
// images maps basename → bytes for files under {page}/ beside page.rm (inserted images).
func RenderV6SVGWithRMC(data []byte, images map[string][]byte) (string, error) {
	...
	if err := stagePageImages(tmpDir, "page", images); err != nil {
		return "", err
	}
...
// stagePageImages writes image blobs under tmpDir/pageName/ so rmc can find them
// via Path(page.rm).with_suffix("").
func stagePageImages(tmpDir, pageName string, images map[string][]byte) error {
	if len(images) == 0 {
		return nil
	}
	dir := filepath.Join(tmpDir, pageName)
